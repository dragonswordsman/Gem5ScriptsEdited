...
parser.add_option("--argument",action="store", type="string", default="0ns",
...
### Configurations
options.instructions = 1000000000
options.flags = "TEST"
...
stats_file = options.binary + "_" + str(options.argument) + "_TRR.txt"
...
\nThe argument of this simulation is " + options.argument)
print("=" * 30 + "\n")

# Specify the output directory
outdir = os.path.expanduser("~/Research/outputs/refresh/")
m5.options.outdir = outdir
m5.core.setOutputDir(outdir)
# Specify the filename of stats
m5.stats.addStatVisitor(stats_file)
# redirect stdout and stderr
stdout_file = "simout_" + options.binary + "_" + options.argument
...
stderr_file = "simerr_" + options.binary + "_" + options.argument
...
for i in range(mem_channels):
    ...
    system.mem_ctrls[i].target_REFI = options.argument
