...
SPEC=$home/spec2k6/benchspec/CPU2006       # Install location of SPEC2006 benchmarks
SIMPOINT=$home/Simpoint
SCRIPT=$home/shellscripts
OUTPUT=$home/outputs                       # Output directory
##################################################################
bench=$1 # First variable in commandline specify the benchmarks selection
...
runscript=$OUTPUT/$bench/run_$bench.log
...
echo " " | tee -a $runscript
echo "=======================Shell Script Starts=======================" | tee -a $runscript
echo " " | tee -a $runscript
echo " " | tee -a $runscript
...
cd $DIR
        ...
        --mem-size=8GB\
        --simpoint-profile\
        --simpoint-interval=10000000| tee -a $runscript
echo "=======================Gem5 Ended========================" | tee -a $runscript
cd $OUTPUT
        cp simpoint.bb.gz sim_$bench.gz
        mv sim_$bench.gz $SIMPOINT

echo " " | tee -a $runscript
echo " " | tee -a $runscript
echo "===========Here goes Simpoit! Starting running!==========" | tee -a $runscript
cd $SIMPOINT
        ./bin/simpoint\
        -loadFVFile sim_$bench.gz\
        -maxK 30\
        -saveSimpoints $bench.simpts\
        -saveSimpointWeights $bench.weights\
        -inputVectorsGzipped true\
echo " " | tee -a $runscript
echo " " | tee -a $runscript
