...
configs = os.path.expanduser("~/Research/ywang-gem5/configs")
addToPath(configs)
...
parser.add_option("--argument",action="store", type=float, default=0,
        help="Para probability after checkpoint restore")
...
### Configurations
options.instructions = 500000000
options.flags = "DBC"
...
# print("Checkpoint Director: " + options.checkpoint_dir + "\n")
...
stats_file = options.binary + "_" + str(options.argument) + "_dbc.txt"
...
will run " + str(options.maxinsts) + " on proposed memory system.\n\
This memory system consists of " + str(options.mem_channels) +" channels. \
\nThe argument of this simulation is " + str(options.argument))
print("=" * 30 + "\n")

# Specify the output directory
outdir = os.path.expanduser("~/Research/outputs/dbc/")
m5.options.outdir = outdir
m5.core.setOutputDir(outdir)
# Specify the filename of stats
m5.stats.addStatVisitor(stats_file)
# redirect stdout and stderr
stdout_file = "simout_" + options.binary + "_" + str(options.argument)
...
stderr_file = "simerr_" + options.binary + "_" + str(options.argument)
...
for i in range(mem_channels):
    ...
    system.mem_ctrls[i].para_prob = options.argument
...
print("Workload " + options.binary + " exits from simulation after simulating " + str(options.instructions))